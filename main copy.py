...
ttk.Label(root, text="选择带有成绩的表格：").grid(row=2, pady=6)
ttk.Label(root, textvariable=student_file_name).grid(row=2, column=2)
ttk.Button(root, text="选择文件", command=chose_file).grid(row=2, column=3, pady=6)
ttk.Label(root, text="选择空表：").grid(row=4, pady=10)
ttk.Label(root, textvariable=empty_file_name).grid(row=4, column=2)
ttk.Button(root, text="选择文件", command=chose_empty_file).grid(row=4, column=3, pady=6)
...
ttk.Button(root, text="确定", width=10, command=show) \
    .grid(row=14, column=0, sticky=tk.W, padx=10, pady=5)

ttk.Button(root, text="取消", width=10, command=sys.exit) \
